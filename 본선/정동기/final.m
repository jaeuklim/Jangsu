...
droneObj = ryze() %드론 객체 지정
cameraObj = camera(droneObj); %카메라 객체 지정
takeoff(droneObj); %드론 이륙
moveup(droneObj, 'distance', 0.4); % 위로 이동

while(1)
    frame = snapshot(cameraObj); %드론 화면 찍기
    if sum(frame, 'all') == 0 %모드 요소의 합이 0이면
        disp('frame error!'); %에러 표시
        continue; %다시 화면 찍기
    end

    src_hsv = rgb2hsv(frame); %hsv로 바꾸기
    src_h = src_hsv(:,:,1); %h
    src_s = src_hsv(:,:,2); %s
    src_v = src_hsv(:,:,3); %v
    [rows, cols, channels] = size(src_hsv); %가로, 세로 구하기 
    ...
    for row = 1:rows
        for col = 1:cols
            if bw1(row, col) == bw2(row, col)   %원에 파란색이 검출된다면
                bw2(row, col) = 0;              %검출 안된걸로 표시
            end
        end
    end

    if sum(bw2, 'all') > 20000 %원이 가까워졌을 때
        % Detecting Center
        ...
        right_cnt = 0; %맨 위에서  0 선언 아래서도 0으로 초기화 해줌
        left_cnt = 0;
        up_cnt = 0;
        down_cnt = 0;
        ...
        if (-100 < moveRow && moveRow < 100) && (-100 < moveCol && moveCol < 100) %픽셀 범위 안에 들어오면
            ...
            if 110000 < bw2_pix_num %원이 가까울 때
                ...
                while (1)
                    ...
                    bw_purple = (thdown_purple(1) < src_h) & (src_h < thup_purple(1)) & (thdown_purple(2) < src_s) & (src_s < thup_purple(2));  % 보라색범위 검출 보라색 범위가 없음          
                    ...
                    elseif(sum(bw_green, "all")> 500)
                        disp('초록색 검출! 각도 변경!')
                        turn(droneObj, deg2rad(90)); %먼저 90도 돌림
                        moveforward(droneObj, 'distance', 1.2);
                        turn(droneObj, deg2rad(45)); %이후 30도 더 돌림
                        right_cnt = 0;
                        left_cnt = 0;
                        up_cnt = 0;
                        down_cnt = 0;
                    ...
                    elseif (sum(bw_purple, 'all') > 1000)                          % 보라색이 검출되면
                        ...
                        elseif up_cnt < 1
                            up_cnt = up_cnt + 1;
                            moveup(droneObj, 'distance', 0.4); %원래자리로 옴
                            continue;
                        elseif left_cnt < 1
                            left_cnt = left_cnt + 1;
                            moveleft(droneObj, 'distance', 0.2);
            ...
            elseif (bw2_pix_num < 110000) %20000보단 크지만 생각보다 작다면 
                disp('원에 조금씩 접근중');
